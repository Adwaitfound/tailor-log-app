...
export default function App() {
  ...
  const [useLocalMode, setUseLocalMode] = useState(false); // <--- THE MAGIC FIX
  ...
  useEffect(() => {
    ...
    const loadLocalData = () => {
      ...
      if (localConfig) {
        ...
        setSetupForm({
          ...
          productsText: (localConfig.products || []).join('\n')
        });
      } else {
        setSetupForm({
      ...
      setProdForm(prev => ({ ...prev, tailor: tailors[0] || '', product: products[0] || '' }));
      setPayForm(prev => ({ ...prev, tailor: tailors[0] || '' }));
      setLoading(false);
    };

    const initFirebaseData = (user) => {
      try {
        ...
        unsubscribeConfig = onSnapshot(doc(db, 'config', 'setup'), (docSnap) => {
          if (docSnap.exists()) {
            ...
            setProdForm(prev => ({ ...prev, tailor: data.tailors?.[0] || '', product: data.products?.[0] || '' }));
    ...
    signInAnonymously(auth).then((cred) => {
      ...
      initFirebaseData(cred.user);
    }).catch((err) => {
      console.warn("Firebase Auth Blocked (Iframe/Cookies). Switching to Local Mode.");
      clearTimeout(fallbackTimeout);
      setUseLocalMode(true);
      loadLocalData();
    });

    // FAILSAFE: If Firebase hangs for 3 seconds, force local mode
    fallbackTimeout = setTimeout(() => {
      if (loading) {
        console.warn("Firebase Timed Out. Forcing Local Mode.");
        setUseLocalMode(true);
        loadLocalData();
        setLoading(false);
      }
